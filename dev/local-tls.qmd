...
# Migrate your Local Dev to HTTPS (TLS)
...
## 🔒 Step 1: Install Certificate Authority (mkcert)
...
## 🛠️ Step 3: Run Next.js Dev Server with HTTPS
...
## 🐳 Step 4: Configure and Restart Keycloak (TLS Profile)
...
3.  **Update `KEYCLOAK_INTERNAL_HOST` in `docker/.env`:**

Services that call Keycloak internally (neo4j-mcp, crisalid-agents-owui, crisalid-agents-chat-api, sovisuplus) resolve it by its container name. When switching to the TLS profile, update this variable so they target the correct container:

```bash
# docker/.env
KEYCLOAK_INTERNAL_HOST=keycloak-tls
```

The default value is `keycloak` (plain HTTP profile). Forgetting this change will cause internal token requests to fail with a connection error.

4.  **Launch Docker Compose with the TLS Profile:**
