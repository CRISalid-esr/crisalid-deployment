...
Before starting IKG for the first time, download the OpenAlex snapshot data (domains, fields,
subfields, and topics). IKG uses these files to build the classification tree in Neo4j on its
first startup, then wipes them automatically.

```bash
bash docker/scripts/download_openalex_tree.sh
```

When IKG starts, it checks the mounted data directory for snapshot files. If files are present, it
runs the import (which is idempotent) and then wipes them so they do not trigger a re-import on the
next startup. To refresh the tree later, re-run the script and restart IKG.
...
from a JSON file, alongside the RBAC one. Start by copying the sample:
...
* Edit `sovisuplus/config/chat.json` to tailor the assistant to your institution. The schema and
  ...
  [SoVisuPlus assistant content](../ai/docker-compose.qmd#sovisuplus-assistant-content).
...
* This step is optional: with no `chat.json` — or one the app rejects — the container falls back to
  the generic `chat.sample.json` baked into the image, and the assistant still works. Check
  `docker compose logs sovisuplus | grep ChatConfigService` if your edits seem to be ignored.
